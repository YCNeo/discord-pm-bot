...
const { DISCORD_TOKEN, BACKLOG_CHANNEL_ID, LOG_CHANNEL_ID, STANDUP_CHANNEL_ID, PM_ROLE_ID } = process.env;
...
client.on('interactionCreate', async (i) => {
    ...
    try {
        if (i.commandName === 'task' && i.options.getSubcommand() === 'create') {
            ...
            if (dueStr && !due) return i.reply({ content: '時間格式錯誤，請用 `YYYY-MM-DD` 或 `YYYY-MM-DD HH:mm`。', ephemeral: true });

            const backlog = await client.channels.fetch(BACKLOG_CHANNEL_ID);
            if (!backlog?.isTextBased() || backlog.type !== ChannelType.GuildText)
                return i.reply({ content: 'BACKLOG_CHANNEL_ID 非文字頻道。', ephemeral: true });

            const embed = new EmbedBuilder().setTitle(`🗂️ ${title}`).setDescription(desc)
                .addFields({ name: '建立者', value: `<@${i.user.id}>`, inline: true }, { name: '狀態', value: 'Backlog', inline: true },
                    ...(due ? [{ name: '截止', value: fmtDate(due), inline: true }] : []))
                .setTimestamp(new Date());

            const msg = await backlog.send({ embeds: [embed] });
            const thread = await msg.startThread({ name: due ? `[${title}] 截止: ${fmtDate(due)}` : `[${title}]`, autoArchiveDuration: 1440 });
            await i.reply({ content: `✅ 任務已建立：<#${thread.id}>`, ephemeral: true });
            await logEvent([`📝 建立任務：${title}`, `By：${i.user.tag}`, `Thread：#${thread.id}`]);
            return;
        }

        if (i.commandName === 'assign') {
            if (!i.channel?.isThread()) return i.reply({ content: '請在任務 Thread 內使用。', ephemeral: true });
            if (!isPMOrManager(i)) return i.reply({ content: '你沒有指派權限。', ephemeral: true });
            const user = i.options.getUser('user', true);
            await i.channel.send(`👤 指派負責人：<@${user.id}>（由 <@${i.user.id}> 指派）`);
            await i.reply({ content: `✅ 已指派給 <@${user.id}>。`, ephemeral: true });
            return;
        }
        ...
        if (i.commandName === 'standup') {
            ...
            if (!ch?.isTextBased()) return i.reply({ content: 'STANDUP_CHANNEL_ID 非文字頻道。', ephemeral: true });
            const today = new Date().toISOString().slice(0, 10);
            await ch.send(['📣 **Daily Standup – ' + today + '**', '請依格式回覆本訊息或開 Thread：', '- 昨天完成：', '- 今天要做：', '- 卡點與需要協助：'].join('\\\\n'));
        ...
        if (i.commandName === 'remind') {
            ...
            if (!ms || ms < 1000) return i.reply({ content: '延遲格式錯誤，請用 10m/2h/1d。', ephemeral: true });
            await i.reply({ content: `🕒 我會在 **${i.options.getString('in')}** 後提醒你。`, ephemeral: true });
            setTimeout(async () => {
                try { (await client.users.fetch(i.user.id)).send(`⏰ 提醒：${msg}`); }
                catch { await logEvent([`⚠️ DM 失敗（${i.user.tag}）`, `內容：${msg}`]); }
            }, ms);
            return;
        }
    } catch (err) {
